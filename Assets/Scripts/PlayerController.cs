...
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (_groundCheck == null) _groundCheck = GetComponentInChildren<GroundCheck>(); // <- se non e`associata, assegno la componente al primo Child
    }

    private void Update()
    {
        if (_groundCheck.IsGrounded) _jumpCount = 0; // <- reset contatore dei salti se siamo a terra
